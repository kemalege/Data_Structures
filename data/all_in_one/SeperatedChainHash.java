...
public class SeperatedChainHash {
    ...
    public static<T> int ara(HashEntry<T>[] dizi, T aranacak) {
        int h=aranacak.hashCode();//Orjinal konum
        int konum = h%dizi.length;
        if(dizi[konum] != null || dizi[konum].isActive || dizi[konum].equals(aranacak))
            return konum;
        else
            return -1;
        
    }
    public static<T> int ara2(HashEntry<T>[] dizi, T aranacak) {
        int h=aranacak.hashCode();//Orjinal konum
        for(int i=0; i<dizi.length; i++){
            if(dizi[i] != null || dizi[i].isActive || dizi[i].equals(aranacak))
                return i;
        }
        return -1;
